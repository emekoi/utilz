...
var zig_c_allocator: *std.mem.Allocator = undefined;

fn initAllocFn() void {
    const root = @import("root");
    if (root != @This() and @hasDecl(root, "zig_c_allocator")) {
        zig_c_allocator = root.zig_c_allocator;
    } else {
...
pub export fn zig_malloc(size: usize) ?*c_void {
    initAlloc.call();
    var bytes = zig_c_allocator.alloc(u8, size + @sizeOf(usize)) catch return null;
...
pub export fn zig_free(ptr: ?*c_void) void {
    ...
    if (ptr) |p| {
        ...
        zig_c_allocator.free(fp[0..size]);
...
pub export fn zig_realloc(ptr: ?*c_void, new: usize) ?*c_void {
    ...
    if (ptr) |p| {
        ...
        var new_ptr = zig_c_allocator.realloc(old_ptr[0..old_size], new + @sizeOf(usize)) catch return null;
